...
describe('Testing state filter "All" on <<Issues>> list', () => {
  it('Tests state filter "All" on table Issues', () => {
...
recurse(
  ...
  },
  {
    ...
    //Test case 15
    //Only items of list that have state "All" should be displayed
    cy.get('tbody tr').each(($el) => {
      cy.wrap($el).within(() => {cy.get('td').eq(3).invoke('text').should('match', /(OPEN|CLOSED)/);
        })
     })
