...
describe('Testing state filter "Closed" of <<Issues>> list', () => {
  it('Tests state filter "Closed" on table Issues', () => {
...
recurse(
  ...
  },
  {
    ...
    cy.get('tbody tr').each(($el) => {
      cy.wrap($el).within(() => {cy.get('td').eq(3).should('have.text', 'CLOSED') 
        })
     })
