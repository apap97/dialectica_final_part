...
describe('Forks list has column "Comments" sorted in ascending order', () => {
  it('Tests if column "Comments" is sorted in ascending order', () => {
...
recurse(
  ...
  },
  {
    ...
    //Test case 35
    //Double click on header "Stars" in order to have elements represended in ascending way
     ...
     cy.get('tbody tr').each(($el) => {
      cy.wrap($el).within(() => {
        cy.get('td').eq(2).invoke('text').as('number')
          cy.get('@number').then((number) => {array_asc.push(Number(number)); }) })
     }).then(()=>{
