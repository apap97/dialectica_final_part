...
describe('Pull Requests list being filtered by state filter "Closed" and has column "Created" sorted in descending order ', () => {
  it('Tests if column "Created" is sorted in descending order while state filter value equals "Closed"', () => {
...
recurse(
  ...
  },
  {
    ...
    //Test case 24
    //Single click on header "Created" in order to have elements represended in descending way
    ...
    cy.get('tbody tr').each(($el) => {
      cy.wrap($el).within(() => {
        cy.get('td').eq(4).invoke('text').as('date')
