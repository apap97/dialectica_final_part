...
describe('Testing state filter "Open" of <<Pull Requests>> list', () => {
  it('Tests state filter "Open" on table Pull Requests', () => {
    ...
    //Test case 26
    //Correct values of state filter: 1)Open(default), 2)Closed, 3)Merged
    cy.get('[class*="styles_filter__1xfmi"]').children().invoke('text').should('eq', 'Show: OpenClosedMerged')
    ...
    cy.wrap($el).within(() => {cy.get('td').eq(2).should('have.text', 'OPEN') 
      })
   })
